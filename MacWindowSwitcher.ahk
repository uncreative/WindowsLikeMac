...
but slightly modified for mac like behavior
	  
HOTKEYS:
  Default:    Cmd+Tab - move forwards in window stack
              Cmd+Shift+Tab - move backwards in window stack
              Cmd+Esc - cancel switching window
...
Initiate_Hotkeys:
  ...
  Hotkey, <#+``, Alt_Shift_Tab, On ; turn on alt-tab hotkey here to be able to turn it off for simple switching of apps in script
  ;#+`::Gosub Alt_Shift_Tab
...
Display_List:
  ...
  ;MsgBox "Display_List" . %Display_List_Shown%
...
Display_List__Find_windows_and_icons:
  ...
  ;MsgBox "Display_List__Find_windows_and_icons"
...
Gui_Window_Group_Delete:
  ...
  IniDelete, MacWindowSwitcher_Settings.ini, Groups, %A_ThisMenuItem%
  IniDelete, MacWindowSwitcher_Settings.ini, Groups, %A_ThisMenuItem%_Group_Hotkey
  Gosub, Alt_Esc_Check_Alt_State ; hides alt-tab gui - shows again if alt still pressed
Return
...
IniFile(Var, Section, Default="")
{
  ...
  If IniFile_Read_or_Write =Read
    {
    IniRead, %Var%, MacWindowSwitcher_Settings.ini, %Section%, %Var%, %Default%
    If %Var% =ERROR
      %Var% = ; set to blank value instead of "error"
    }
  Else If IniFile_Read_or_Write =Write
    IniWrite, % %Var%, MacWindowSwitcher_Settings.ini, %Section%, %Var%
}
...
Delete_Ini_File_Settings:
  ...
  IfMsgbox, Cancel
    Return
  FileDelete, MacWindowSwitcher_Settings.ini
