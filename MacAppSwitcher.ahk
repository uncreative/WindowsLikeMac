;#include C:\Users\uncreative\code\autohotkey\
#Include AHKA.ahk

;SetWorkingDir, C:\Users\uncreative\code\autohotkey\
...
Display_List__Find_windows_and_icons:
  ...
  Loop, %Window_List%
    ...
    found := AHKA_Find(execsArray, Exe_Name, 1)
    If (found > 0) {
      ;MsgBox "dup " %Exe_Name%
      Continue
    }
    len = AHK_Size( execsArray )
    ;MsgBox "adding " %Exe_Name% " len: " %len% " found: " %found%
    execsArray := AHKA_Add(execsArray, Exe_Name)
...
Gui_Window_Group_Delete:
  ...
  IniDelete, MacAppSwitcher_Settings.ini, Groups, %A_ThisMenuItem%
  IniDelete, MacAppSwitcher_Settings.ini, Groups, %A_ThisMenuItem%_Group_Hotkey
  Gosub, Alt_Esc_Check_Alt_State ; hides alt-tab gui - shows again if alt still pressed
Return
...
IniFile(Var, Section, Default="")
{
  ...
  If IniFile_Read_or_Write =Read
    {
    IniRead, %Var%, MacAppSwitcher_Settings.ini, %Section%, %Var%, %Default%
    If %Var% =ERROR
      %Var% = ; set to blank value instead of "error"
    }
  Else If IniFile_Read_or_Write =Write
    IniWrite, % %Var%, MacAppSwitcher_Settings.ini, %Section%, %Var%
}
...
Delete_Ini_File_Settings:
  ...
  IfMsgbox, Cancel
    Return
  FileDelete, MacAppSwitcher_Settings.ini
